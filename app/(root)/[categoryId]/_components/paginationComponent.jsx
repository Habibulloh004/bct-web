  ...
  return (
    <Pagination>
      <PaginationContent className="w-full flex flex-wrap justify-center gap-1 sm:gap-2 py-2">
        ...
        {pages.map((p, index) =>
          p === "..." ? (
            <PaginationItem key={index} className="hidden sm:block border rounded-md">
              <PaginationEllipsis />
            </PaginationItem>
          ) : (
            <PaginationItem key={index}>
              <PaginationLink
                ...
                className={cn(`border w-8 h-8 sm:w-10 sm:h-10 flex items-center justify-center text-sm sm:text-base`, currentPage === p ? "bg-primary hover:bg-priamry hover:text-white text-white":"hover:bg-primary/10")}
              >
                {p}
              </PaginationLink>
            </PaginationItem>
