...
export function createCategoryMetadata({ category, categoryId, page = 1 }) {
  ...
  const categoryPath = category ? createCategoryPath(category) : `/${extractMongoId(categoryId) || categoryId}`;
...
export function createProductMetadata({ product, categoryId, productId }) {
  ...
  const path = product
    ...
    : `/${extractMongoId(categoryId) || categoryId}/${extractMongoId(productId) || productId}`;
